...
class UpFile
{
    ...
    private function validate()
    {
        ...
        if (!$this->chkSize()) {$this->error('为空或大小超出允许 '.$this->byteFormat($this->maxSize));return false;}
        if (!$this->chkType()) {$this->error('类型超出允许，允许类型：'.$this->getAllowExt());return false;}
