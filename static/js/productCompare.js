jQuery(document).ready(function() {
    ...
    compareBox += '        <ul></ul>';
    compareBox += '    </div>';
	compareBox += '    <div class="CompareBut clearfix">';
    compareBox += '    <a class="ShowCompareSubmit fl" id = "ShowCompare" title="比较分析" href="#" ></a><a id="ClearAllProduct" class="ClearCompare fr"></a>';
	compareBox += '    </div>';
    compareBox += '</div>';
    ...
    jQuery(".addCompare").each(function(index) {
        jQuery(this).click(function() {
            ...
            if (IsExistProduct(jQuery(this).attr("value"))) {
                ShowMsg("已经存在该线路！", this);
                return false;
            }
            if (productCount >= 3){
            	ShowMsg("最多只能选3条线路！", this); 
                return false;
            }
    ...
    function ClearAllProduct() {
        ...
        jQuery("#ShowProductCount").text("线路总数:0");
    ...
    function AddProduct(productID, productName, imgSrc, link) {
        ...
        html += '<a href="' + link.replace(".php",".php?") + '" title="查看线路详情">' + productName + '</a>';
        html += '</li>';
        ...
        jQuery("a.removeProduct").each(function() {
            jQuery(this).click(function() {
                ...
                jQuery("#ShowProductCount").text("线路总数:" + productCount);
    ...
    jQuery(".ShowCompareSubmit").click(function(event) {
        if (event.target == this) {
            ...
            var url = "index.php?mod=compare&id=";
